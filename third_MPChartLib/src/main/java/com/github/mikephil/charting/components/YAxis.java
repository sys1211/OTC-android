...
public class YAxis extends AxisBase {
    ...
    private float defaultValueOffset = 1;

    public void setDefaultValueOffset(float defaultValueOffset) {
...
//        float width = (float) Utils.calcTextWidth(p, label) + getXOffset() * 2f;
        // addition change double x offset to single x offset
    ...
    public void calculate(float dataMin, float dataMax) {
        ...
        if (range == 0f) {
            max = max + defaultValueOffset;
            min = min - defaultValueOffset;
        }
