...
public class BarLineChartTouchListener extends ChartTouchListener<BarLineChartBase<? extends BarLineScatterCandleBubbleData<?
    ...
    private void performDrag(MotionEvent event, float distanceX, float distanceY) {
        if (mChart.isHighlightScrollOnly()) {
            mLastGesture = ChartGesture.DRAG;
    ...
    public int getLastScaleIndex() {
        return lastScaleIndex;
    }

    protected final float[] matrixBuffer = new float[9];

    private float getScale(float totalDist, float mSavedXDist) {
        float maxScaleX = mChart.getViewPortHandler().getMaxScaleX();
        float minScaleX = mChart.getViewPortHandler().getMinScaleX();
        float offset = Math.abs(minScaleX - maxScaleX);
        float scaleCount = mChart.getScaleXCount();
        float step = offset / scaleCount;

        mSavedMatrix.getValues(matrixBuffer);
        float originalScaleX = matrixBuffer[Matrix.MSCALE_X];

//        Log.e("toggleScaleStep", "pinch zoom originalScaleX : " + originalScaleX);
        // TODO + 0.5 for 2.999999 to int is 2 , however had should be 3
        int currentIndex = (int) ((originalScaleX - minScaleX) / step + 0.5);

        int targetIndex = (int) (currentIndex + (totalDist - mSavedXDist) / 150);

        targetIndex = Math.max(Math.min(targetIndex, (int) scaleCount - 1), 0);

        Log.e("toggleScaleStep", "currentIndex : " + currentIndex + " targetIndex : " + targetIndex);


        if (currentIndex != targetIndex) {
            float targetScale = originalScaleX + (targetIndex - currentIndex) * step;
            if (targetScale > maxScaleX) {
                targetScale = maxScaleX;
            }

            if (targetScale < minScaleX) {
                targetScale = minScaleX;
            }

            lastScaleIndex = targetIndex;

            return targetScale / originalScaleX;
        }
    ...
    private float getScale(int index) {
        ...
        if (index > scaleCount - 1 || index < 0) {
            return 1;
        }

        float originalScaleX = mChart.getViewPortHandler().getScaleX();
        // TODO + 0.5 for 2.999999 to int is 2 , however had should be 3
        int currentIndex = (int) ((originalScaleX - minScaleX) / step + 0.5);

        Log.e("toggleScaleStep", "originalScaleX " + originalScaleX);

        Log.e("toggleScaleStep", "maxScaleX" + (maxScaleX) + ",minScaleX" + (minScaleX) + ",currentIndex" + (currentIndex) + ",targetIndex " + (index));


        if (currentIndex != index) {
            float targetScale = originalScaleX + (index - currentIndex) * step;
            if (targetScale > maxScaleX) {
                targetScale = maxScaleX;
            }
            ...
            Log.e("toggleScaleStep", "targetScale " + targetScale);
            return targetScale / originalScaleX;
        }
    ...
    public void toggleScaleStep(int index) {
        if (mChart.getScaleXCount() == 0) {
            return;
        }
...
//        Log.e("toggleScaleStep", "index : " + index + " ; scale : " + scale);

        lastScaleIndex = index;

        mMatrix.set(mChart.getViewPortHandler().getMatrixTouch());
        mMatrix.postScale(scale, 1f, 0, 0);
...
//            l.onChartScale(event, scaleX, 1f);
        ...
        float scaleX = matrixBuffer[Matrix.MSCALE_X];

//        Log.e("toggleScaleStep", "after toggle scale : " + scaleX);
    }
        ...
        if (event.getPointerCount() >= 2) { // two finger zoom
            ...
            if (totalDist > mMinScalePointerDistance) {
                ...
                if (mTouchMode == PINCH_ZOOM) {
                    ...
                    if (mChart.getScaleXCount() > 0) {
                        scaleX = getScale(xDist, mSavedXDist);
                    }
